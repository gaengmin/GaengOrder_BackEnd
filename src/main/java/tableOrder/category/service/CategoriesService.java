...
public class CategoriesService {
    ...
    /**
     * 카테고리명을 수정하는 메소드
     * @param updateCategory 카테고리 번호와 변경할 이름 정보를 담은 DTO
     * @throws IllegalArgumentException 해당 카테고리가 존재하지 않거나, 이름 변경이 실패한 경우
     */
    public void changeCategoriesName(RequestCategoryDto.UpdateCategory updateCategory) {
        int updatedRows = categoriesMapper.changeCategoriesName(updateCategory);
        if (updatedRows == 0) {
    ...
    * */
    ...
    public void softDeleteCategory(RequestCategoryDto.SoftDeleteCategory softDeleteCategory) {
        Long categoryNo = softDeleteCategory.getCategoryNo();
